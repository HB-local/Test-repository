    ...
    for await (const line of lineInterface) {
        ...
        if (repoInfo.iStatement >= 0) {
            ...
            if (iRepoPerformance >= 0) {
                const rest = repoInfo.statement.substring(iRepoPerformance + repoPerformance.length);
                const duration = timeRegex.exec(rest);
                if (duration !== null) {
        ...
        if (sqlInfo.statement && sqlInfo.performance) {
            // write it into file!!!
            sqlInfo.outStream.write(sqlInfo.statement + symbols.tab + sqlInfo.performance + symbols.newLine);
            sqlInfo.statement = "";
            sqlInfo.performance = "";
            sqlInProcess = false;
        }
        ...
        if (repoInfo.statement && repoInfo.performance) {
            repoInfo.outStream.write(repoInfo.statement +
                symbols.tab +
...
//# sourceMappingURL=ProcessLogV1.js.map